...
class App extends Component {
  ...
  add = data => {
    ...
    //console.log(data.CitizenName, data.PrivateKey, data.email;)
    var name = data.CitizenName + " " + data.PublicKey;
    certcontract.methods.addcert(name, data.PrivateKey, data.Email).send(
      {
      ...
      (error, result) => {
        if (error) console.log("error " + error);
          ...
          this.setState({ name: data.CitizenName + " " + data.PublicKey });
          this.setState({ PrivateKey: data.PrivateKey + " " + "PrivateKey" });
